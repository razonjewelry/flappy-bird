...
export default function TaskRow({ task, onDelete, onPickDate, partnerPhone, isNew }) {
  ...
  const saveEdit = () => {
    ...
    setTaskText(task.id, text).catch(() => setIsEditing(false));
  };
  ...
  return (
    <View style={[styles.row, task.isImportant && !done && styles.rowImportant, done && styles.rowDone]}>
      ...
      {!done && (
        <>
          ...
          <TouchableOpacity
            ...
            hitSlop={8}
            style={styles.iconButton}
            accessibilityLabel="קבע תאריך"
          >
            <Icon
              name="calendar"
              size={19}
              color={task.dueAt ? colors.primary : colors.muted}
            />
          </TouchableOpacity>
      ...
      <TouchableOpacity onPress={onDelete} hitSlop={8} style={styles.iconButton} accessibilityLabel="מחק">
